...
# 3. Buat skrip startup otomatis untuk menembak IP Minikube
RUN echo '#!/bin/bash' > /entrypoint.sh && \
    echo 'if [ -f /root/.kube/config ]; then' >> /entrypoint.sh && \
...
# 4. Ganti trigger nyala Jenkins menggunakan skrip di atas
